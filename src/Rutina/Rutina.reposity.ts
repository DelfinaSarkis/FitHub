...
export class RutinaRepository {
  constructor(
    @InjectRepository(Rutina)private readonly rutinaRepository: Repository<Rutina>, @InjectRepository(Category)private categoryRepository:Repository<Category>, @InjectRepository(Users)private userRepository:Repository<Users>, @InjectRepository(Ejercicio)private exerciceRepository:Repository<Ejercicio>) {}
  async getAllRutinas(page: number, limit: number, category?:string, location?:string, difficultyLevel?:string, search?:string) {
    let whereConditions: any = { isActive: true, check: true };
        ...
        if (category) {
          const categoria = await this.categoryRepository.findOne({where:{id:category}});
  ...
  async createRutina(rutina:CreateRutinaDto,userId:string) {
    ...
    if(!category.length){throw new BadRequestException('Categoria no encontrada')}

    const exercise = await this.exerciceRepository.find({where:{id:In(rutina.exercise),user:{id:userId}},relations:['user']});
    
    if(!exercise.length){throw new BadRequestException('Ejercicio no encontrado')}
    return await this.rutinaRepository.save({...rutina,admin,category,exercise});//Este metodo cambie por create
  }
  ...
  async deleteRutina(id, user) {
    ...
    if (!rutina || rutina.isActive === false) { throw new NotFoundException('Plan no encontrado o eliminado')};

    if(user.role !== UserRole.ADMIN){
      const userSolicitud = await this.userRepository.findOne({where:{id:user.sub}});
      if (rutina.admin.id!==user.id) { throw new BadRequestException('No tines capacidad de eliminar este plan')}
